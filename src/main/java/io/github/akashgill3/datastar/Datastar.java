 ...
 *
 * <p>The primary responsibility is to safely create {@link DatastarSseEmitter} instances while
 * enforcing the maximum concurrent connection limit defined in configuration. When the limit is
 * exceeded, an {@link IllegalStateException} is thrown.
 *
 * <p>Each emitter is automatically registered with lifecycle callbacks to track active connections
 * and log events (if debug logging is enabled).
 *
 * <p>Typical usage:
 *
 * <pre>{@code
...
public class Datastar {
  ...
  /**
   * Creates a new {@link DatastarSseEmitter} instance with connection tracking and lifecycle
   * callbacks.
   *
   * <p>This method atomically increments the active connection count and validates it against the
   * maximum concurrent connection limit specified in the configuration. If the limit is exceeded,
   * the connection count is decremented and an {@link IllegalStateException} is thrown.
   *
   * <p>The returned emitter is automatically configured with:
   *
   * <ul>
   *   <li>Completion callback - decrements active connections and logs if debug logging is enabled
   *   <li>Timeout callback - decrements active connections and logs a warning
   *   <li>Error callback - decrements active connections and logs the error with stack trace
   * </ul>
   *
   * <p>Thread-safe for concurrent invocations.
   *
  ...
  public DatastarSseEmitter createEmitter() {
    ...
    emitter.onTimeout(
        () -> {
          ...
          if (properties.enableLogging() && log.isDebugEnabled()) {
            log.debug(
                "SSE connection timed out. Active connections: {} (If unexpected, review server/proxy idle timeouts or send keep-alives.)",
